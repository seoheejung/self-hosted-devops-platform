...
> GitLab Runner 기반으로 GitLab Repository의 Pipeline 실행 흐름을 검증하고, Docker Compose 설정 검증 및 배포 단계 Job 실행 가능 여부를 확인한다.
...
Phase 5에서는 GitLab Runner를 기반으로 GitLab Repository의 Pipeline 실행 구조를 검증한다.
...
Mini PC
 └─ Docker 접근 및 Docker Compose config 검증
```
...
deploy-check
 ├─ 배포 단계 Job 실행 가능 여부 확인
 └─ 배포 대상 Compose 파일 존재 여부 확인
```
...
| Stage | 역할 |
| --- | --- |
| `validate` | Repository 구조와 Docker Compose 설정 검증 |
| `deploy-check` | 실제 배포 전 배포 단계 Job 실행 가능 여부 확인 |
...
Phase 5에서는 별도 `test` stage를 구성하지 않는다.

Runner 실행 흐름은 `validate-compose`와 `deploy-readiness-check` Job이 Runner에서 정상 실행되는지로 확인한다.
...
현재 GitLab 서버 이미지는 직접 build하지 않는다.

GitLab Omnibus Container는 Phase 1 ~ Phase 3에서 구성한 고정 버전 이미지를 사용하며, Phase 5에서는 GitLab Repository의 Pipeline 실행 구조와 Docker Compose config 검증에 집중한다.
...
GitLab Pipeline에서 배포 단계 Job이 Runner에서 실행 가능한지 확인한다.
...
Phase 5에서는 실제 Docker Compose 배포 명령을 실행하지 않고, Runner가 배포 단계 Job을 정상 실행할 수 있는지만 확인한다.

```
GitLab Pipeline Deploy Check Job
 → GitLab Runner
 → 배포 단계 실행 가능 여부 확인
...
| Deploy Check Job | 실제 배포 전 배포 단계 실행 가능 여부 확인 |
| Docker Build Job | 직접 build 대상이 생긴 뒤 추가 |
| 실제 Deploy Job | Phase 5 범위에서는 수행하지 않음 |
...
- `deploy-readiness-check` Job이 Runner에서 실행됨
- 실제 배포 명령은 실행하지 않음
- 기존 GitLab Container에 영향을 주지 않음
- 배포 대상 Compose 파일 존재 여부를 확인함
